...
@app.post("/deposit/")
async def deposit(transaction: Transaction, authorization: str = Header(None)):
...
@app.post("/withdraw/")
async def withdraw(transaction: Transaction, authorization: str = Header(None)):
    current_user = get_current_user(authorization)
    ...
    if current_balance >= transaction.amount:
        ...
        return {
            ...
            "balance": read_balance(current_user),
        }
    else:
...
async def register(authorization: str = Header(None)):
    ...
    cursor.execute(
        "INSERT INTO accounts (username, balance) VALUES (?, ?)", (current_user, 0)
    )
    conn.commit()
...
async def delete_user(username: str, authorization: str = Header(None)):
    ...
    if current_user != username:
        raise HTTPException(
            status_code=401, detail="You can only delete your own account"
        )
